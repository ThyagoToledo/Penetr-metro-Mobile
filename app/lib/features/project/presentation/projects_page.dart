      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => showProjectEditDialog(context, ref),
          ),
        ],
      ),
      body: async.when(
        ...
        data: (projects) {
          if (projects.isEmpty) {
            return _EmptyState(
              onCreate: () => showProjectEditDialog(context, ref),
            );
          }
...
class _ProjectCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final count = measurements.where((m) => m.projectId == project.id).length;
    ...
    return Card(
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: Icon(
            project.archived ? Icons.inventory_2_outlined : Icons.folder,
          ),
        ),
        title: Text(project.name),
        subtitle: Text('$count medição(ões) · $description'),
  ...
  Future<void> _onAction(
    ...
    switch (action) {
      case 'edit':
        await showProjectEditDialog(context, ref, existing: project);
      case 'duplicate':
